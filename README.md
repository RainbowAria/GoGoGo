...
- 可选 KataGo 人类风格模型：职业 1–9 段对应 `rank_1d`–`rank_9d` 棋谱策略
...
安装人类风格模型时，程序从相应 `rank_1d`–`rank_9d` 策略概率中选择合法
落点，并用主网络搜索判断何时虚手以及估计胜率；未安装时选择主网络最高评价
落点。职业段位是面向训练和娱乐的模拟档位，不是棋院认证，也无法保证与每位
真实职业棋手的实际水平完全一致。
