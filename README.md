...
- KataGo AI 分析工作台：查看候选着、胜率、领先目数、局面控制热图和多分支变化
- 中国数子法自动计分，白方贴 6.5 目
...
在未结束的棋局中点击“AI 分析”或按 `F4`。工作台使用 KataGo 分析独立副本；
如果正在与训练模型对弈，仍使用本局选定的训练模型，其余对局使用 KataGo 设置中的主网络。
打开期间正式棋局暂停，关闭后从原局面继续。

右侧显示候选着的黑方胜率、领先目数、搜索次数和后续变化，棋盘显示候选点与
局面控制热图。可以双击候选、在工作台棋盘手动推演、展开完整变化，或沿变化树
回看；`F5` 会重新分析当前节点。正式棋局和工作台的分支互不影响。
